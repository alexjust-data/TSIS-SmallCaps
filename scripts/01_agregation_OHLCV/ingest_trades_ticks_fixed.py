...
# Usaremos hora UTC equivalente (ET - 4/5 horas segÃºn DST)
# Simplificado: usar rangos de hora del dÃ­a
...
def load_tickers(path: str) -> List[str]:
    ...
    if path.endswith('.parquet'):
        ...
        log(f"ExtensiÃ³n desconocida, intentando como CSV: {path}")
        df = pl.read_csv(path)
    ...
    log(f"Cargados {len(tickers):,} tickers Ãºnicos")
...
def build_session() -> requests.Session:
    s = requests.Session()
    # Pool mÃ¡s grande para concurrencia real
...
def normalize_trades(results: list, ticker: str, trade_date: str, args=None) -> tuple:
    """
    Normaliza trades y separa en premarket/market segÃºn timestamp
...
def fetch_and_stream_write_trades(session: requests.Session, api_key: str, ticker: str,
    ...
    """Descarga trades para un dÃ­a especÃ­fico y escribe directamente
    ...
    # LÃ­mites de rate adaptativo (bajamos el mÃ­nimo)
...
def get_directory_size(path: Path) -> int:
    """Calcula tamaÃ±o total de directorio en bytes"""
    total = 0
    try:
...
def main():
    ap = argparse.ArgumentParser(description="Descarga trades tick-level (streaming por dÃ­a)")
    ...
    ap.add_argument("--rate-limit", type=float, default=0.18, help="segundos entre pÃ¡ginas")
    ap.add_argument("--max-tickers-per-process", type=int, default=15,
                    help="Max. tickers que procesarÃ¡ este proceso antes de salir")
    ap.add_argument("--max-workers", type=int, default=1, help="(IGNORADO) Paralelismo lo maneja el launcher")
    # Opciones nuevas:
    ap.add_argument("--skip-weekends", action="store_true", help="Saltar sÃ¡bados y domingos")
    ...
    # Lista mÃ­nima de festivos USA (amplÃ­ala si quieres)
    US_HOLIDAYS = {
    ...
    # Iterar por DÃAS (descarga diaria)
    ...
    for t in tickers:
        try:
            ...
            for day in date_range(df, dt0):
                ...
                # === CHECKPOINTING: Verificar si dÃ­a ya completado ===
                ticker_dir = outdir / t
                year_dir = ticker_dir / f"year={day.year}"
                month_dir = year_dir / f"month={day.month:02d}"
                day_dir = month_dir / f"day={day_str}"
                success_marker = day_dir / "_SUCCESS"

                # === Caso: dÃ­a ya completo ===
                if success_marker.exists():
                    try:
                        premarket_fp = day_dir / "premarket.parquet"
                        market_fp = day_dir / "market.parquet"
                        if premarket_fp.exists():
                            pm = pl.read_parquet(premarket_fp)
                            rows_sum += len(pm)
                        if market_fp.exists():
                            mk = pl.read_parquet(market_fp)
                            rows_sum += len(mk)
                    except Exception:
                        pass
                    days_sum += 1
                ...
                # === Caso: dÃ­a con parquet(s) pero sin _SUCCESS â†’ posible dÃ­a parcial â†’ re-descargar ===
                premarket_fp = day_dir / "premarket.parquet"
                market_fp = day_dir / "market.parquet"

                if (premarket_fp.exists() or market_fp.exists()) and (not success_marker.exists()):
                    log(f"  {t} {day_str}: parquet(s) found but no _SUCCESS â†’ re-downloading to ensure integrity")
                ...
                # === Marcar dÃ­a como completado si al menos un parquet existe ===
                premarket_fp = day_dir / "premarket.parquet"
                market_fp = day_dir / "market.parquet"
                if premarket_fp.exists() or market_fp.exists():
                    success_marker.touch()  # Crear marcador _SUCCESS

                # Log progreso cada 200 dÃ­as
                if days_sum > 0 and days_sum % 200 == 0:
                    log(f"  {t}: {days_sum}/~2555 dÃ­as | {rows_sum:,} trades")
