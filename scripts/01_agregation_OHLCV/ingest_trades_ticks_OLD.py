    ...
    for t in tickers:
        try:
            ...
            for day in date_range(df, dt0):
                ...
                # === CHECKPOINTING: Verificar si día ya completado ===
                ticker_dir = outdir / t
                ...
                # === Caso: día con parquet(s) pero sin _SUCCESS → posible día parcial → re-descargar ===
                premarket_fp = day_dir / "premarket.parquet"
                market_fp = day_dir / "market.parquet"

                if (premarket_fp.exists() or market_fp.exists()) and (not success_marker.exists()):
                    log(f"  {t} {day_str}: parquet(s) found but no _SUCCESS → re-downloading to ensure integrity")
                ...
                # === Marcar día como completado si al menos un parquet existe ===
                premarket_fp = day_dir / "premarket.parquet"
                market_fp = day_dir / "market.parquet"
                if premarket_fp.exists() or market_fp.exists():
